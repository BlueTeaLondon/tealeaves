...
- New generator: `tealeaves:single_redirect` for setting up `Rack::CanonicalHost`
- New generator: `tealeaves:production:compression` for setting up `Rack::Deflater`
- New generator: `tealeaves:preloader` for managing spring
- New generator: `tealeaves:advisories` for installing bundler-audit
- New generator: `tealeaves:profiler` for setting up rack_mini_profiler
- New generator: `tealeaves:runner` for making an app runnable locally
...
- Upgraded: Rails 6.0.
- New generator: `tealeaves:inline_svg` for setting up the inline_svg gem.
...
- New generator: `tealeaves:stylelint` for setting up stylelint.
- New generator: `tealeaves:production:manifest` for app.json.
- New generator: `tealeaves:production:deployment` for bin/deploy script.
...
- New generator: `tealeaves:json` for JSON parsing.
- New generator: `tealeaves:staging:pull_requests` for Heroku app per PR.
...
- Bug fix: the tealeaves gem itself is not needed in production
- Bug fix: bundle install after adding a gem
- Breaking: remove Refills
- Breaking: rename tealeaves:enforce_ssl to tealeaves:production:force_tls
