...
module Tealeaves
  class AppBuilder < Rails::AppBuilder
    ...
    def gitignore
      copy_file "tealeaves_gitignore", ".gitignore"
    end
    ...
    def replace_gemfile
      template "Gemfile.erb", "Gemfile", force: true do |content|
        if development_env?
          content.gsub(%r{gem .tealeaves.}) { |s| %(#{s}, path: "#{root_path}") }
        else
          content
        end
      end
    ...
    def ruby_version
      create_file ".ruby-version", "#{Tealeaves::RUBY_VERSION}\n"
    end
    ...
    def development_env?
      root_path.join("tealeaves.gemspec").exist?
    end
